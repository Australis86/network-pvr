...
#
# Created:     06/07/2015
# Copyright:   (c) Joshua White 2015-2018
...
receipient = '' # Email receipient
ssmtp_path = '/usr/sbin/ssmtp' # Path to ssmtp binary
ISP_RELAY = '' # ISP SMTP relay (set to None if not applicable)
SENDER = '' # Email address to put in "from" field
...
def sendEmail(subject, dest=receipient, text='', html='', useisp=(ISP_RELAY is not None), account=None, passw=None):
    '''Send the email. If smtplib is present and you provide either your ISP's
    SMTP relay or a gmail account and password, it will send using the relay or
    via that account. Otherwise, it will send through sstmp, if it is set up.'''

    if html == '':
        html = text

    if SMTP_available and (useisp or (account and passw)):
        # Create message container - the correct MIME type is multipart/alternative.
        outer = MIMEMultipart('alternative')
        msg1 = MIMEText(text, 'plain') # Plaintext
        msg2 = MIMEText(html, 'html') # HTML
        outer.attach(msg1)
        outer.attach(msg2)
        outer.add_header('reply-to', dest)

        outer['Subject'] = subject
        outer['From'] = SENDER
        outer['To'] = dest

        email_msg = outer.as_string()

        # Send the message via the selected SMTP server
        if useisp:
            s = smtplib.SMTP(ISP_RELAY)
            s.sendmail(SENDER, dest, email_msg)
            s.quit()
            ...
            s.sendmail(account, dest, email_msg)
            s.quit()

        logPrint('Sent email using smtplib.')

    elif not SMTP_available:
        # Use subprocess here
        email = '''MIME-Version: 1.0\nContent-Type: text/html\nTo: <%s>\nFrom: "%s" <%s>\nReply-To: <%s>\nSubject: %s\n\n%s''' % (dest, SENDER, dest, dest, subject, text)
        ...
        cmdstr = '%s %s < "%s"' % (ssmtp_path, dest, EMAILFILE)
        try:
            subprocess.call(cmdstr, shell=True)
            os.remove(EMAILFILE)
            logPrint('Sent email using ssmtp subprocess.')

        except Exception, err:
            logPrint('Error sending email via ssmtp binary:\n%s' % str(err))

    else:
        logmsg = 'Unable to send email.'

        if not SMTP_available:  logmsg += '\n\tssmtplib not available.'
        if not useisp:          logmsg += '\n\tNo ISP selected.'
        if not account:         logmsg += '\n\tNo Gmail account provided.'
        if not passw:           logmsg += '\n\tNo Gmail password provided.'
